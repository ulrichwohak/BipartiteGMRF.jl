@testset "solvers" begin
    ...
    ss_limited = suffstats_synthetic()
    ...
    rvs = solve(model_vs, ss_vs, ExactCholesky(optim_iters=2); seed=1)
    @test isfinite(rvs.nll)

    ss_sp = suffstats_synthetic(BipartiteSpectralModel)
    model_sp = BipartiteSpectralModel(ss_sp.A_prior)
    @test_throws ArgumentError solve(model_sp, ss_sp, ExactCholesky(optim_iters=2))
end
...
@testset "g_reltol convergence tolerance" begin
    ...
    tight = fit_mle(BipartiteNormalizedModel, d.f, d.w, d.y;
                    solver=HutchSLQ(; base..., g_reltol=1e-7),
                    seed=1, verbose=false)
    loose = fit_mle(BipartiteNormalizedModel, d.f, d.w, d.y;
...
@testset "VS + ExactCholesky on acyclic graphs (issue #82)" begin
    ...
    td = tree_data()
    ss_tree = suffstats(BipartiteVarianceStableModel, td.f, td.w, td.y)
    model_tree = BipartiteVarianceStableModel(ss_tree.A_prior)
    @test BipartiteGMRF.is_forest(model_tree.graph.A)
    res = solve(model_tree, ss_tree, ExactCholesky(optim_iters=200, polish=true); seed=1)
    @test res.converged
    ...
    ss_cyc = suffstats_synthetic(BipartiteVarianceStableModel)
    ...
    rce = solve(model_cyc, ss_cyc, ExactCholesky(optim_iters=10); seed=1)
