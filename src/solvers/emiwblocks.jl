# ═══════════════════════════════════════════════════════════════════════════
# Free per-firm error blocks, integrated-likelihood version (issue #112, §0
# remedy E): Ω_i are nuisance realizations drawn iid from an inverse-Wishart
# population law and integrated out — never estimated. Scalar scale-mixture
# form: u_i ~ Gamma(δ/2, δ/2) per firm, ε_i | u_i ~ N(0, Ψ_{m_i}/u_i) with
# Ψ_m = φ·[(1−r)I + r𝟙𝟙']. Variational EM under q(α)·∏q(u_i); every update is
# closed-form (r and δ by 1-D searches) and reuses the dense-verified
# emfreeblocks machinery (assembly, workspaces, posterior moments, θ score).
# ═══════════════════════════════════════════════════════════════════════════
...
# catastrophically once x ≳ 1e6 (loggamma ~ x·log x dwarfs the O(h·log x)
# result); the Stirling form is exact to O(h³/x²) there.
...
3. One safeguarded score step on `(ρ, σ_a, σ_z)`: the θ-dependent ELBO part is
   `½logdet K − ½tr(K S_α)`, whose gradient is the same Fisher-identity form
   the free-block EM uses.
...
function optimize_emiw(
    ...
    fb !== nothing || throw(ArgumentError("optimize_emiw requires error_blocks=:free statistics."))
    nf = stats.N_firms
    nw = stats.N_workers
    ...
    r = solver.r isa Float64 ? clamp(solver.r, r_lo, 1.0 - 1e-8) : 0.0
    ...
    while iterations < solver.max_iter
        ...
        P, b = assemble_freeblock_precision(fb, Ωeff, nf, nw)
        ...
        # (dense-verified in the free-block EM; q(α)-expectations, θ-free).
        resid = fb.y - V * μ
        for i in 1:B
            rr = rows[i]
            ri = resid[rr]
        ...
        # with the δ update to their joint fixed point. The (q(u), δ) pair is
        # the slowest coordinate direction (δ's profile drifts as q(u)
        # tightens); one alternation per outer iteration makes δ crawl for
        # hundreds of iterations, whereas the inner loop converges it in a
        # handful. Both updates only touch O(B) scalars.
        ...
        # gradient is the Fisher-identity form _emfree_score computes (with
        # ws_M holding the fixed variational P). Armijo on −F, monotone. A few
        # inner steps per iteration: candidates only cost a numeric ws_Q
        # update + one selinv_dot on the already-factored ws_M, and pushing θ
        # further per E-step shortens the EM ridge crawl.
        for _ in 1:4
            gψ = _emfree_score(model, ew, μ, ψ, limit)
        ...
        # (δ is updated jointly with q(u) in step 2; capped at DELTA_CAP,
        # where the t-blocks are Gaussian to numerical precision — δ at the
        # cap means "no detectable block heterogeneity".)
    ...
    abs(rho) < 1e-2 && @warn(
        "Integrated free-block fit landed at small |ρ| = $(abs(rho)): σ_a is identified " *
...
function _emiw_theta_step(
    ...
    for _ in 1:80
        ψn = ψ0 .- step .* gψ
        fn = negF(ψn)
        # Armijo sufficient decrease (not any decrease): near a stationary
        # point, accepting noise-level improvements makes θ random-walk on the
        # flat region instead of settling.
