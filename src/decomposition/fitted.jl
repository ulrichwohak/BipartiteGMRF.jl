function _decompose_fitted(
    ...
    p = unpack_params(result.theta_unconstrained; rho_limit=rho_limit(problem.model))
    ...
    if result.solver isa ExactCholesky
        ...
        mop = MOp(qop, problem.VtV, zeros(n), lambda)
        ws = PCGWorkspace(n)
        Mdiag = q_diag(problem, p.rho, sigma_a, sigma_z) .+ lambda .* Vector{Float64}(diag(problem.VtV))
    ...
    th_f = view(theta_hat, 1:problem.N_firms)
    th_w = view(theta_hat, problem.N_firms + 1:n)
    tmp = Vector{Float64}(undef, problem.N_firms)
